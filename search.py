...
# --- Extraction Logic (Votre code original conservé) ---
	...
	match root.name:
		...
		case "quote":
			...
			for node in root:
				...
				extract_text_inner(node, buf)
		case _:
			raise Exception("unsupported: {root!r}")
...
def get_titles(doc):
	# Correction : chemin XPath correct pour les titres
	titles = doc.find("/document/title")
	return [extract_text(t) for t in titles]

def prepare_search_data(doc):
	data = {}
	data["identifier"] = get_identifier(doc)
	data["logical"] = get_logical(doc)
	data["title"] = get_titles(doc)
